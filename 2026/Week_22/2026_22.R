## Challenge: #MakeoverMonday 2026 week 22
## Data:      Arsenal 25/26 Champions
## Author:    Steven Ponce
## Date:      2026-06-02

## Article
# https://www.arsenal.com/news/arsenal-analysed-how-we-won-premier-league

## Source
# https://fbref.com/en/squads/18bb7c10/Arsenal-Stats

## Data
# https://data.world/makeovermonday/2026w22-arsenal-2526-champions
...
## 2. READ IN THE DATA ----
df_raw <- read_csv("data/2026/premier_league_player_match_stats.csv") |>
  clean_names()
...
p_cliff <- ggplot(arsenal_buckets, aes(x = bucket, y = points, fill = bucket)) +
  ...
  annotate(
    "richtext",
    x = cliff_note_x, y = cliff_note_y,
    label = str_glue(
      "**53 of 85 points**<br>",
      "62% of Arsenal's title-winning points<br>came in clean-sheet matches<br>",
      "<span style='font-size:9pt;color:{col_neutral}'>17–2–0 when conceding zero</span>"
    ),
    color = col_dark, family = fonts$text, size = 3.4,
    hjust = 0.5, vjust = 1, lineheight = 1.2, fill = NA, label.color = NA
  ) +
  # Scales
  scale_fill_manual(
  ...
  plot_annotation(
    ...
    theme = theme(
      ...
      plot.caption = element_markdown(
        family = fonts$caption, size = rel(0.55), color = col_neutral,
...
# pandoc   NA
# quarto   ERROR: Unknown command "TMPDIR=C:/Users/poncest/AppData/Local/Temp/Rtmp4YV5e5/filea2e8592c3303". Did you mean command "install"? @ C:\\Users\\poncest\\AppData\\Local\\Programs\\Quarto\\bin\\quarto.exe
# 
# ─ Packages ─────────────────────────────────────────────────────────────────────────
